...
int main(int argc, char* argv[])
{
    ...
    {
        KeyPoint p1 = keypoints1[matches[matchInd].queryIdx];
        KeyPoint p2 = keypoints2[matches[matchInd].trainIdx];

        double dist = sqrt(pow(p1.pt.x - center.x,2) +
            pow(p1.pt.y - center.y,2));

        double angle = atan((center.y - p1.pt.y)/(center.x-p1.pt.x));
        double delta_a = M_PI / 180 * (p2.angle - p1.angle);
        double delta_scale = p2.size / p1.size;

        double dAngle = delta_a + angle;
        double dR = delta_scale * dist;

        Point2f newCenter(p2.pt.x + dR * cos(dAngle),
                 p2.pt.y + dR * sin(dAngle));
        hgt.at<int>( newCenter.y / splitSet , newCenter.x / splitSet ) += 1;
    }
